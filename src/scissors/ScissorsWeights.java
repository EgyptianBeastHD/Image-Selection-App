...
public class ScissorsWeights {
    ...
    static Weigher<ImageEdge> makeWeigher(String weightName, ImageGraph graph) {
        return switch (weightName) {
            ...
            case "ColorWeigher" -> new ColorWeight(graph);
            case "LuminanceWeigher" -> new LuminanceWeight(graph);
    ...
    static Iterable<String> weightNames() {
        return List.of("CrossGradMono", "ColorWeigher", "LuminanceWeigher");
    }
    ...
    /**
     * Calculates the weight of an edge based on the color intensities along its path.
     * This method favors edges that align with lower changes in color and penalizes
     * those that cross high color gradients.
     */
    static class ColorWeight implements Weigher<ImageEdge> {
        ...
        /**
         * The raster of the image represented by `graph`.
         */
        ...
        public int weight(ImageEdge edge) {
            ...
            for (int i = 0; i < colorImage.getNumBands(); i++) {
                grad += crossGrad(colorImage, x, y, i, edge.dir());
            }

            grad /= colorImage.getNumBands();

            int eGradMax = ((edge.dir() % 2) == 0) ? 180 : 255;

            return eGradMax - grad;
        }
    }

    /**
     * Creates a new weigher capable of weighing edges in `graph` by converting its image
     * to a grayscale image based on luminance perception.
     */
    static class LuminanceWeight implements Weigher<ImageEdge> {
        /**
         * The graph that the edges to be weighed will come from.
         */
        private ImageGraph graph;
         ...
         * Create a new weigher capable of weighing edges in `graph`.
         */
        LuminanceWeight(ImageGraph graph) {
            this.graph = graph;
            Raster src = graph.raster();
            // Rec. 601 colors instead of Rec. 709 because mcgraw tower is not a super high quality picture
            float r = 0.299f;
            float b = 0.587f;
            float g = 0.114f;
            float[][] matrix = {
        ...
        public int weight(ImageEdge edge) {
            ...
            int grad = crossGrad(grayImage, x, y, 0, edge.dir());
