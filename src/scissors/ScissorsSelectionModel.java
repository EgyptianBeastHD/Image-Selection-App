...
public class ScissorsSelectionModel extends SelectionModel {
    ...
    public PolyLine liveWire(Point p) {
        int dstId = graph.idAt(p);
        List<Integer> path = paths.pathTo(dstId);
    ...
    private class ShortestPathsWorker
        ...
        public PathfindingSnapshot doInBackground() {
            PathfindingSnapshot path = null;
            while (!pathfinder.allPathsFound()) {
                if (isCancelled()) {
                    return null;
                }
                path = pathfinder.extendSearch(10000);
                int progress = (int) (100.0 * pathfinder.settledCount() / pathfinder.vertexCount());
        ...
        public void done() {
            if (worker == this) {
                try {
                    ...
                    if (previousState == SELECTING || previousState == NO_SELECTION) {
                        undoPoint();
                        setState(previousState);
                    }
