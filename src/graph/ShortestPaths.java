...
public class ShortestPaths<VertexType extends Vertex<EdgeType>, EdgeType extends Edge> {
    ...
    /**
     * `predecessors[id]` is the ID of the penultimate vertex along the shortest known path from
     * our starting vertex to the vertex with ID `id`, or -1 if `id==startId` or if no such path
     * is currently known.
     */
    private final int[] predecessors;

    /**
     * Queue of vertex IDs currently known to be reachable from the starting vertex but for whom
     * the shortest possible path has not yet been determined.  Ordered by weight of the
     * shortest known path from the starting vertex.
     */
    ...
    /**
     * Create a new shortest paths solver for the graph `graph` whose edge weights are
     * determined by `weigher`.
     */
    ...
    /**
     * Return the number of vertices for which the final shortest paths from the current
    ...
    /**
     * Change our starting point to `start`, discarding any pathfinding results from any
     ...
     * returned object, which will also provide candidate paths to farther "frontier" vertices
     * found during the pathfinding process.
     * <p>
     * If fewer than `maxToSettle` reachable vertices do not have the shortest paths to them
     * known, then this method returns when shortest paths have been found for all reachable
     * vertices in the graph.  If multiple vertices tie for being the next closest, it is
     * unspecified which will have its shortest path found first.  Requires `maxToSettle` is
     * non-negative and that a starting vertex has been set.
     */
    public PathfindingSnapshot extendSearch(int maxToSettle) {
        assert startId >= 0;
        assert maxToSettle >= 0;
        int initialSize = settledCount();
        frontier.addOrUpdate(startId, 0);

        while (!frontier.isEmpty() && (settledCount() - initialSize) < maxToSettle) {
            int currentId = frontier.remove();
            Vertex<EdgeType> startVertex = graph.getVertex(currentId);
            // settling the current vertex
            if (!settledIds.get(currentId)) {
                settledIds.set(currentId);
            }
            for (EdgeType v : startVertex.outgoingEdges()) {
                int dist = distances[currentId] + weigher.weight(v);
                if (distances[v.endId()] < 0) {
                    frontier.addOrUpdate(v.endId(), dist);
                    predecessors[v.endId()] = currentId;
                    distances[v.endId()] = dist;
                } else if (distances[v.endId()] > dist) {
                    frontier.addOrUpdate(v.endId(), dist);
                    predecessors[v.endId()] = currentId;
                    distances[v.endId()] = dist;
                }
