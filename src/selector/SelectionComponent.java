...
public class SelectionComponent extends JComponent implements MouseListener, MouseMotionListener,
    ...
    private void paintControlPoints(Graphics g, List<PolyLine> segments) {
        ...
        for (PolyLine segment : segments) {
            for (int i = 0; i < segment.size(); i++) {
                g.fillOval(segment.xs()[i] - controlPointRadius,
                        segment.ys()[i] - controlPointRadius,
    ...
    private void paintMoveGuides(Graphics g, List<PolyLine> segments) {
        ...
        if (selectedIndex < segments.size() - 1) {
            after = segments.get(selectedIndex + 1).start();
        } else {
            after = segments.get(0).start();
        }
        if (selectedIndex > 0) {
            before = segments.get(selectedIndex - 1).start();
        } else {
            before = segments.get(segments.size() - 1).start();
        }

        g.setColor(liveWireColor);
        g.drawLine(mouseLocation.x, mouseLocation.y, before.x, before.y);
        g.drawLine(mouseLocation.x, mouseLocation.y, after.x, after.y);
    }
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1 && model.state() == SELECTED) {
            ...
            int closestIndex = model.closestPoint(point, controlPointRadius);
            if (closestIndex != -1) {
                selectedIndex = closestIndex;
                updateMouseLocation(point);
                repaint();
            }
        }
    }
