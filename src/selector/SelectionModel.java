...
public abstract class SelectionModel {
    ...
    public int closestPoint(Point p, int maxDistanceSq) {
        ...
        int closestIndex = -1;
        int minDistanceSq = Integer.MAX_VALUE; // Initialize with the largest possible value
        int i = 0;
        for (PolyLine segment: selection()) {
            Point startPoint = segment.start();
            int distanceSq = ((startPoint.x - p.x) * (startPoint.x - p.x) + (startPoint.y - p.y) * (
                    startPoint.y - p.y));
